...
public class IteratorApplication {
  ...
  public static void main(String[] args) {
    ...
    Payment payment =
        Payment.builder().paymentType(PaymentType.CREDIT_CARD).amount(1_000_000L).build();

    context.getBeansOfType(PaymentService.class).values().stream()
        .filter(paymentService -> paymentService.isSupport(payment.getPaymentType())).findFirst()
